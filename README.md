...
### Vault: deposit, mint, stake, unstake, withdraw, redeem for oneWayMarket
```ts
    await llamalend.init('JsonRpc', {});
    await llamalend.oneWayfactory.fetchMarkets();
...
### Create loan, add collateral, borrow more, repay for oneWayMarket
```ts
(async () => {
    await llamalend.init('JsonRpc', {});
...
### Create loan all ranges methods for oneWayMarket
```ts
    await llamalend.init('JsonRpc', {});
...
### Self-liquidation for oneWayMarket
```ts
(async () => {
    await llamalend.init('JsonRpc', {});
...
### User loss for oneWayMarket
```ts
(async () => {
    await llamalend.init('JsonRpc', {});

    const oneWayMarket = llamalend.getoneWayMarket('sfrxeth');
...
### Leverage (createLoan, borrowMore, repay) for oneWayMarket
```ts
(async () => {
...
### Leverage createLoan all ranges methods for oneWayMarket
```ts
    await llamalend.init('JsonRpc', {}, {}, API_KEY_1INCH);
    await llamalend.oneWayfactory.fetchMarkets();
...
import {llamalend} from "./lending";

(async () => {
    await llamalend.init('JsonRpc', {});

    const llamma = llamalend.getLlamma('eth');
...
### Create loan, add collateral, borrow more, repay for llamma
```ts
(async () => {
    await llamalend.init('JsonRpc', {});

    console.log(llamalend.getLlammaList());
    // [ 'sfrxeth' ]
...
### Create loan all ranges methods for llamma
```ts
    await llamalend.init('JsonRpc', {});
...
### User loss for llamma
```ts
(async () => {
    await llamalend.init('JsonRpc', {});

    const llamma = llamalend.getLlamma('sfrxeth');
...
(async () => {
    ...
    const llamma = llamalend.getLlamma('wsteth');
...
### Leverage all ranges methods for llamma
```ts
    await llamalend.init('JsonRpc', {});

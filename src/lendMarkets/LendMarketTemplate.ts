...
import {createRouterAdapter, RouterAdapterType} from "../routerAdapters/index.js";
import ERC20Abi from '../constants/abis/ERC20.json' with {type: 'json'};
import {cacheKey, cacheStats} from "../cache/index.js";
import {TRouterName} from "../constants/routers";
...
export class LendMarketTemplate {
    ...
    leverage: {
        ...
        createLoanRouteImage: (userBorrowed: TAmount, debt: TAmount, router: TRouterName) => Promise<string>,
    ...
    private leverageCreateLoanMaxRecvAllRanges = memoize(async (userCollateral: TAmount, userBorrowed: TAmount, router: TRouterName):
        ...
        const routerMethods = this._getRouterMethods(router);
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if (pAvgBN === null){
                const _y = BigInt(await routerMethods.getExpected(
                    this.addresses.borrowed_token, 
                    this.addresses.collateral_token, 
                    _maxBorrowable[0], 
                    this.addresses.amm
    ...
    private _getSwapDataFromCache = (inputCoinAddress: string, _amount: bigint, router: TRouterName): IQuoteOdos => {
        const key = `${router}-${inputCoinAddress}-${_amount}`;
    ...
    private async leverageCreateLoanRouteImage(userBorrowed: TAmount, debt: TAmount, router: TRouterName): Promise<string> {
        this._checkLeverageZap();
        const _userBorrowed = parseUnits(userBorrowed, this.borrowed_token.decimals);
        const _debt = parseUnits(debt, this.borrowed_token.decimals);
    ...
    private async _leverageCreateLoan(
        ...
        if (slippage !== swapData.slippage) throw Error(`You must call leverage.createLoanExpectedCollateral() with router="${router}" and slippage=${slippage} first`);
        const calldata = swapData.pathId as string;
        const contract = this.llamalend.contracts[this.addresses.controller].contract;
